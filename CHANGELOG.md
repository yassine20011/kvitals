...
- **Data-Driven Metric Pipeline**: Replaced manual sensor wiring with a centralized architecture (`MetricDefinitions.js`, `MetricConfig.qml`, `MetricStore.qml`, `HardwareDiscovery.qml`, `ViewHelpers.js`). Views now consume normalized metric lists without knowledge of sensor internals.
- **Metric Contract Normalization**: Enforced a boundary separating Quantitative metrics (scalar values, chart histories, threshold evaluation) from Display-Only metrics (IP addresses, uptime strings), with input sanitization against NaN, Infinity, null, and non-numeric types.
- **Unified Hardware Discovery**: Centralized hardware querying into `HardwareDiscovery.qml`, maintaining exactly one `SensorTreeModel` and `KDescendantsProxyModel` per active QML context with debounced topology updates.
- **Per-GPU Sub-Metric Configuration**: Added granular sub-metric selection and custom label overrides per discovered GPU.
- **Single Source of Truth for Sub-Metric Defaults**: Centralized default sub-metric lists in `MetricDefinitions.GROUPS`, eliminating hardcoded duplicates across QML modules.

### Changed

- **Sensor Polling Isolation**: Sensor modules now focus strictly on data acquisition and domain formatting, delegating threshold calculations, visibility checks, and chart history buffering to `MetricStore`.
- **View Presentation Separation**: `CompactView` and `FullView` delegates now receive pre-formatted items generated by `ViewHelpers.js` according to `MetricConfig.orderedKeys`.
- **Contributor Documentation**: Restructured architecture and contribution guides into explicit two-tier workflows for adding sub-metrics versus full hardware categories.
